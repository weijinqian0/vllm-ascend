...
def async_all_gather(input_, group, event=None, is_use_get_global_memory_buffer=False):
    ...
    ag_out = torch.empty(dim_size, dtype=input_.dtype, device=torch.npu.current_device())
    if event:
...
def async_reduce_scatter(input_, group, event=None, stream=None, is_use_get_global_memory_buffer=False):
    ...
    rs_out = torch.empty(dim_size, dtype=input_.dtype, device=torch.npu.current_device())
    if event or stream:
        # multi stream wait event
        global COMM_STREAM
        if COMM_STREAM is None:
