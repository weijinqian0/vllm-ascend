...
def fused_experts_with_all2allv(token_dispatcher, probs, routing_map, hidden_states: torch.Tensor,
    ...
    (share_experts_output, hidden_states, tokens_per_expert) = token_dispatcher.token_permutation(
    ...
    hidden_states = apply_mlp(hidden_states,
                              ...
                              w2,
                              tokens_per_expert)
    hidden_states, mlp_bias = token_dispatcher.token_unpermutation(hidden_states)
    return hidden_states
...
class AscendFusedMoE(FusedMoE):
    ...
    def __init__(
        ...
        if VLLM_ASCEND_ENABLE_MOE_ALL2ALLV and isinstance(self.quant_method, AscendUnquantizedFusedMoEMethod):
